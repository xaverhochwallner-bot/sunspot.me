...
def shadow():
    try:
        ...
        print(f"🌍 [DEBUG] /shadow — Zoom: {zoom}", flush=True)
...
def shadow_stream():
    ...
    print(f"🌍 [DEBUG] /shadow/stream — Zoom: {zoom}", flush=True)
    min_lat = request.args.get("minLat", default=None,    type=float)
    min_lon = request.args.get("minLon", default=None,    type=float)
    max_lat = request.args.get("maxLat", default=None,    type=float)
    max_lon = request.args.get("maxLon", default=None,    type=float)
    ...
    def generate():
        try:
            ...
            if zoom <= MACRO_ZOOM_THRESHOLD:
                # Invalidate cache if stored compute area doesn't cover current viewport_bbox
                # (prevents postage-stamp shadow when cache was populated by a smaller viewport)
                if ck in _shadow_cache:
                    entry = _shadow_cache[ck]
                    if isinstance(entry, tuple) and len(entry) > 3:
                        q_cached = entry[3]
                        compute_box = shapely_box(q_cached[1], q_cached[0], q_cached[3], q_cached[2])
                        if not compute_box.covers(viewport_bbox):
                            del _shadow_cache[ck]
